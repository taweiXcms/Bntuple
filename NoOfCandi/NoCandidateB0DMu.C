...
void dimuonLoop(float ptmin, float ptmax, float ymin, float ymax)
{
  ...
  for(j=0;j<nentries;j++)
    {
      ...
      for(k=0;k<size;k++)
	{
	  if(pt[k]>ptmin&&pt[k]<ptmax&&y[k]>ymin&&y[k]<ymax&&(HLT_PAMu3_v1)&&abs(mumumass[k]-3.096916)<0.15&&mass[k]>5&&mass[k]<6&&trk1Pt[k]>0.7&&trk2Pt[k]>0.7&&chi2cl[k]>9.94e-02&&(d0[k]/d0Err[k])>6.08&&cos(dtheta[k])>7.93e-01&&abs(tktkmass[k]-0.89594)<0.10&&(gen[k]==23333||gen[k]==410000))
	    {
	      realsize++;
	      /*
  ...
  for(j=0;j<nentriesData;j++)
    {
      ...
      for(k=0;k<size;k++)
	{
	  if(pt[k]>ptmin&&pt[k]<ptmax&&y[k]>ymin&&y[k]<ymax&&(HLT_PAMu3_v1)&&abs(mumumass[k]-3.096916)<0.15&&mass[k]>5&&mass[k]<6&&trk1Pt[k]>0.7&&trk2Pt[k]>0.7&&chi2cl[k]>9.94e-02&&(d0[k]/d0Err[k])>6.08&&cos(dtheta[k])>7.93e-01&&abs(tktkmass[k]-0.89594)<0.10&&mu1pt[k]>1.5&&mu2pt[k]>1.5&&abs(mass[k]-5.279)<0.04)
	    {
	      realsizeData++;
	      /*	      
  ...
  c1->SaveAs("CandidateResult/Bzero_candidates_compare_mupt.pdf");

}

void NoCandidateB0DMu()
{
