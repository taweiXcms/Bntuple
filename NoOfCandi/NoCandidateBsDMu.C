...
void dimuonLoop(float ptmin, float ptmax, float ymin, float ymax)
{
  ...
  for(j=0;j<nentries;j++)
    {
      ...
      for(k=0;k<size;k++)
	{
	  if(pt[k]>ptmin&&pt[k]<ptmax&&y[k]>ymin&&y[k]<ymax&&(HLT_PAMu3_v1)&&abs(mumumass[k]-3.096916)<0.15&&mass[k]>5&&mass[k]<6&&trk1Pt[k]>0.7&&trk2Pt[k]>0.7&&chi2cl[k]>3.71e-02&&(d0[k]/d0Err[k])>3.37&&cos(dtheta[k])>2.60e-01&&abs(tktkmass[k]-1.019455)<1.55e-02&&gen[k]==23333)
	    {
	      realsize++;
	      /*
  ...
  for(j=0;j<nentriesData;j++)
    {
      ...
      for(k=0;k<size;k++)
	{
	  if(pt[k]>ptmin&&pt[k]<ptmax&&y[k]>ymin&&y[k]<ymax&&(HLT_PAMu3_v1)&&abs(mumumass[k]-3.096916)<0.15&&mass[k]>5&&mass[k]<6&&trk1Pt[k]>0.7&&trk2Pt[k]>0.7&&chi2cl[k]>3.71e-02&&(d0[k]/d0Err[k])>3.37&&cos(dtheta[k])>2.60e-01&&abs(tktkmass[k]-1.019455)<1.55e-02&&mass[k]-5.367)<0.04)
	    {
	      realsizeData++;
	      /*	      
  ...
  */
  ...
  c1->SaveAs("CandidateResult/Bs_candidates_compare_mupt.pdf");

}

void NoCandidateBsDMu()
{
