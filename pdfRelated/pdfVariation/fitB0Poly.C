...
//   TH1D *hBck = new TH1D(Form("hBck%d",count),"",40,5,6);
   ...
   TF1 *f = new TF1(Form("f%d",count),"[0]*([7]*Gaus(x,[1],[2])/(sqrt(2*3.14159)*[2])+(1-[7])*Gaus(x,[1],[8])/(sqrt(2*3.14159)*[8]))+[3]+[4]*x+[5]*x*x+[6]*("+iNP+")");
...
//   cout <<mass->Integral(0,1.2)<<" "<<mass->IntegralError(0,1.2)<<endl;
 ...
 //  hBck->Draw("hist same");
   ...
   c->SaveAs(Form("PDFVariation/data/PolyChange/ResultsBzero/BMass-%d.pdf",count));
   //c->SaveAs(Form("PDFVariation1Bin/data/PolyChange/ResultsBzero/BMass-%d.pdf",count));

   return mass;
}

void fitB0Poly(TString infname="",bool doweight = 1)
{
