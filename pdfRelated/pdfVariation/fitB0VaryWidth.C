...
TF1 *fit(TTree *nt, TTree *ntMC, double ptmin,double ptmax,double frac)
{   
   static int count=0;
   count++;
   ...
   f->FixParameter(2,f->GetParameter(2)*(1+frac));
   f->FixParameter(7,f->GetParameter(7));
   f->FixParameter(8,f->GetParameter(8)*(1+frac));
   ...
   c->SaveAs(Form("PDFVariation/data/widthvary/ResultsBzero/BMass-%d.pdf",count));
   //c->SaveAs(Form("PDFVariation1Bin/data/width1p2/ResultsBzero/BMass-%d.pdf",count));

   return mass;
}

void fitB0VaryWidth(TString infname="",bool doweight = 1)
{
  ...
  const int nBins = 7;
  double fracBins[nBins+1] = {-0.3,-0.2,-0.1,0,0.1,0.2,0.3};

  TH1D *hPt = new TH1D("hPt","",7,-0.35,0.35);
  for (int i=0;i<nBins;i++)
    {
      TF1 *f = fit(nt,ntMC,10.,60.,fracBins[i]);
  ...
  hPt->SetXTitle("Fraction of width variation");
  hPt->SetYTitle("Signal candidate number");
  hPt->Sumw2();
  hPt->Draw();
  ...
  c->SaveAs("PDFVariation/data/widthvary/ResultsBzero/widthvary.pdf");
