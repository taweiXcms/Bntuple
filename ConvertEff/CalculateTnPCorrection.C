...
TString inputdata_kp="/afs/cern.ch/work/g/ginnocen/nt_20140801_mixed_fromQMBFinder_Kp.root";
...
void CalculateTnPCorrection(){

  TFile *inf;
  TTree *nt;
  if(_type==0) inf = new TFile(inputdata_kp.Data());
  if(_type==1)inf = new TFile(inputdata_kstar.Data());
  if(_type==2) inf = new TFile(inputdata_phi.Data());
