...
//TString cut="(HLT_PAMu3_v1)&&abs(mumumass-3.096916)<0.15&&mass>5&&mass<6&&isbestchi2&&trk1Pt>0.7&&trk2Pt>0.7&&chi2cl>3.71e-02&&(d0/d0Err)>3.37&&cos(dtheta)>2.60e-01&&abs(tktkmass-1.019455)<1.55e-02&&mu1pt>1.5&&mu2pt>1.5&&mass>5.35&&mass<5.39";
TString cut="(HLT_PAMu3_v1)&&abs(mumumass-3.096916)<0.15&&mass>5&&mass<6&&trk1Pt>0.7&&trk2Pt>0.7&&chi2cl>3.71e-02&&(d0/d0Err)>3.37&&cos(dtheta)>2.60e-01&&abs(tktkmass-1.019455)<1.55e-02&&mu1pt>1.5&&mu2pt>1.5&&mass>5.35&&mass<5.39";
...
void dimuonLoop(float ptmin, float ptmax, float ymin, float ymax, int i)
{
  ...
  TH1D* hData = new TH1D("hData","",25,1.,1.05);
  TH1D* hMC = new TH1D("hMC","",25,1.,1.05);
  ...
  c->SaveAs(Form("TktkResult/Bs_tktkmass_dataMC.pdf",i));
}
