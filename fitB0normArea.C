...
//   TH1D *hBck = new TH1D(Form("hBck%d",count),"",40,5,6);
   ...
   TF1 *f = new TF1(Form("f%d",count),Form("([0]/%s)*([7]*Gaus(x,[1],[2])/(sqrt(2*3.14159)*[2])+(1-[7])*Gaus(x,[1],[8])/(sqrt(2*3.14159)*[8]))+[3]+[4]*x+([6]/122.204)*%s",normSignal.Data(),iNP.Data()));
   ...
   TF1 *Bkpi = new TF1(Form("fBkpi",count),Form("([0]/122.204)*%s",iNP.Data()));
...
void fitB0normArea(TString infname="",bool doweight = 1)
{
