...
TF1 *fit(TTree *nt, TTree *ntMC, double ptmin,double ptmax,int count)
{   
   //cout<<cut.Data()<<endl;
   //static int count=0;
   //count++;
...
void fitB0_extend(TString infname="",bool doweight = 1)
{
  ...
  TFile *outf = new TFile("../ResultsBzero/SigmaBzero_extend.root","recreate");

  for (int i=0;i<nBins+1;i++)
    {
      if (i==nBins) {fit(nt,ntMC,10,60,i);continue;}
      TF1 *f = fit(nt,ntMC,ptBins[i],ptBins[i+1],i);
