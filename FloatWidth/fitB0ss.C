...
//   TH1D *hBck = new TH1D(Form("hBck%d",count),"",40,5,6);
   ...
   TF1 *fMC = new TF1(Form("fMC%d",count),"[0]*([7]*Gaus(x,[1],[2])/(sqrt(2*3.14159)*[2])+(1-[7])*Gaus(x,[1],[8])/(sqrt(2*3.14159)*[8]))+[3]+[4]*x+[6]*("+iNP+")");
   TF1 *f = new TF1(Form("f%d",count),"[0]*([7]*Gaus(x,[1],[2]*(1+[5]))/(sqrt(2*3.14159)*[2])+(1-[7])*Gaus(x,[1],[8]*(1+[5]))/(sqrt(2*3.14159)*[8]))+[3]+[4]*x+[6]*("+iNP+")");
   ...
   fMC->SetParLimits(2,0.01,0.08);
   fMC->SetParLimits(8,0.01,0.1);
   fMC->SetParLimits(7,0,1);
   fMC->SetParLimits(6,0,1000);
   ...
   cout<<"==========================================="<<endl;
   cout<<f->GetParameter(5)<<"  "<<f->GetParError(5)<<endl;
   cout<<"==========================================="<<endl;
...
//   cout <<mass->Integral(0,1.2)<<" "<<mass->IntegralError(0,1.2)<<endl;
 ...
 //  hBck->Draw("hist same");
   ...
   c->SaveAs("signalWidth/BMass-B0-AllPt-FloatWidth.pdf");

   return mass;
}

void fitB0ss(TString infname="",bool doweight = 1)
{
