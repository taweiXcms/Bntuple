...
void checkB0(){
  ...
  Int_t nentries = (Int_t)nt->GetEntries();
   ...
   for (i=0; i<nentries; i++) {
	 ...
	 bool cut3=(HLT_PAMu3_v1[j]==1);
	 bool cut4=(mass[j]>5.);
	 bool cut5=(mass[j]<6.);
	 bool cut6=(isbestchi2[j]);
	 bool cut7=(trk1Pt[j]>0.7);
	 bool cut8=(trk2Pt[j]>0.7);
	 bool cut9=(chi2cl[j]>(1.65e-1));
	 bool cut10=((d0[j]/d0Err[j])>4.16);
	 bool cut11=(cos(dtheta[j])>(7.50e-1));
	 bool cut12=abs(tktkmass[0]-0.89594)<2.33e-01;
   ...
   canvassecond->SaveAs("canvassecond.png");
}  
