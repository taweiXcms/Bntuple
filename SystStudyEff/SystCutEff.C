...
void fitB(int index)
{
  ...
  TFile *outf = new TFile(Form("ResultsBplus/SigmaBplus%d.root",index),"recreate");
  outf->cd();
  hPt->Write();
  hEff->Write();
  hPtGen->Write();
...
void SystCutEff(){
  ...
  for (int i=0;i<steps;i++){
    ...
    cutvalue=i*stepvalue;
    cut=Form("(HLT_PAL1DoubleMu0_v1||HLT_PAMu3_v1)&&chi2cl>0.0054&&(d0)/d0Err>3.3&&cos(dtheta)>-0.53&&TMath::Abs((trk1Dxy)/trk1D0Err)>%f",cutvalue);
