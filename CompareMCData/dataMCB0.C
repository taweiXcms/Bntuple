...
TString cut="(HLT_PAMu3_v1)&&abs(mumumass-3.096916)<0.15&&mass>5&&mass<6&&trk1Pt>0.7&&trk2Pt>0.7"; 
//TString cut="(HLT_PAMu3_v1)&&abs(mumumass-3.096916)<0.15&&mass>5&&mass<6&& isbestchi2&&trk1Pt>0.7&&trk2Pt>0.7&&chi2cl>1.65e-01&&(d0/d0Err)>4.16&&cos(dtheta)>7.50e-01&&abs(tktkmass-0.89594)<2.33e-01"; 
...
void dataMCB0(char * var="d0/d0Err",char *title="d0/#sigma(d0)",char *fileTitle="d0d0err",int nBin=20,double BinL=0,double BinH=200,bool
logy=0)
{
//abs(mass-5.279)<0.04&&
    ...
    TTree *tData = (TTree*) infData->Get("ntKstar");
    ...
    TTree *tMC = (TTree*) infMC->Get("ntKstar");
...
//    TFile *infP = new TFile("/data/bmeson/MC/nt_BoostedMC_20140404_Hijing_PPb502_MinimumBias_HIJINGemb_JPsiWithFSR_5TeV_skim.root");
    ...
    TTree *tP = (TTree*) infP->Get("ntKstar");
...
//    double normMC = 163./1349.;  // based on # of B+ candidate
...
//    tMC->Draw("d0/d0Err>>hMC",seldata_2y && massCut && TCut("gen==22233"));    
    ...
    tMC->Draw(Form("%s>>hMCBck",var),(seldata_2y && massCut && TCut("gen!=22233&&gen!=41000")));    
...
//TCut("(1.88912-0.0761755*pt+0.00111135*pt*pt)*(0.9+0.4358*(y+0.465)+0.1339*(y+0.465)**2-0.243*(y+0.465)**3)")*
    ...
    c->SaveAs(Form("figureB0/DataMC-%s.pdf",fileTitle));
    c->SaveAs(Form("figureB0/DataMC-%s.C",fileTitle));
