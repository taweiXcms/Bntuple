...
void fillTree(TVector3* bP, TVector3* bVtx, TLorentzVector* b4P, int j, int typesize, float track_mass1, float track_mass2, int REAL, int PbpMC)
{
  ...
  if(b4P->Pt()==0) cout<<"debug jpsi"<<endl;
  ...
  if(BInfo_type[j]==1 || BInfo_type[j]==2)
  ...
  else if(BInfo_type[j]==5)
    {
      ...
      if(b4P->Pt()==0) cout<<"debug doublet"<<endl;
  ...
  if(!REAL)
      ...
      if(gen[typesize]==23333 || gen[typesize]==41000)
	{
	  ...
	  //if(b4P->Pt()==0) cout<<"Debug GenInfo"<<endl;
	...
	for (int j=0;j<GenInfo_size;j++)
	  {
	    ...
	    if(bGen.Pt()==0) cout<<"Debug GenFill"<<endl;
	    Geneta[j] = bGen.Eta();
	    Genphi[j] = bGen.Phi();
	    Genpt[j] = bGen.Pt();
	    ...
	    GenisSignalBzeroToKzero1270[j] = GenFindisSignal(j,2,511,321,211,10313) || GenFindisSignal(j,2,511,211,321,10313);
	  }
	ntGen->Fill();    
	}
    */
